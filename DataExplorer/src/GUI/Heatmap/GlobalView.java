...
public class GlobalView extends JFrame implements IPlot {
	...
	public GlobalView(String name, ClusterNode columns,ClusterNode rows) {
		super(name);
		create(name, columns.getFirstOrder(), rows.getFirstOrder());
	...
	public GlobalView(String name, Vector Experiments,
			Vector Genes) {
		super(name);

		create(name,Experiments,Genes);

	}
	
	
	public void create(String name, Vector Experiments,Vector Genes) {
...
//		}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getButton() == MouseEvent.BUTTON3 || e.isControlDown()) {
			...
			if (Rows.elementAt(0) instanceof Case
			...
			if (exp != null && gene != null) {
				...
				if (Aggregation == 1) {
					...
					for (int i = 0; i < geneVars.size(); i++) {
						if (gene instanceof Case && ((Case)gene).annGene != null) s += "<FONT FACE = 'Arial'><TR><TD>"+geneVars.elementAt(i).name+ " " + geneVars.elementAt(i).getStringData(((Case)gene).annGene.ID) + "  </TD><TD> ";
					}
	...
	public void sortGenesByChrPosition() {
		...
		Vector<Vector<Case>> Chromosomes = new Vector();

		for (int i = 0; i < Rows.size(); i++) {
			Case gene = (Case) Rows.elementAt(i);
			String chr = "NA";
			...
			Vector<Case> genes = findChrInList(Chromosomes, chr);
	...
	public static Vector<Vector<Case>> sortChromosomes(
			Vector<Vector<Case>> stringBuffer) {

		Vector<Vector<Case>> newBuffer = new Vector();
			...
			for (int j = 0; j < stringBuffer.size(); j++) {
				...
				if (Tools.compareLexicoChr(s1, s2)) {
					if (i < j) {
						...
						Vector<Case> o = (Vector<Case>) newBuffer.elementAt(j);
	 ...
	 */
	public void insertGene(Case gene, Vector<Case> genes) {
		boolean insert = true;
	...
	public Vector<Case> findChrInList(Vector<Vector<Case>> Chromosomes, String s) {
		for (int i = 0; i < Chromosomes.size(); i++) {
			String name = "NA";
